...
function OverviewScreen({ mode }: { mode: LoadMode }) {
  ...
  const selectedPeriod = dashboardPeriodRange(anchorDate, granularity, customRange);
  const queryKey = `${granularity}:${selectedPeriod.start.format("YYYY-MM-DD")}:${selectedPeriod.end.startOf("day").format("YYYY-MM-DD")}`;
  ...
  const loadDashboardMetrics = useCallback(async () => {
    ...
    try {
      const [, requestAnchorDate, requestEndDate] = queryKey.split(":");
  ...
  return (
    <>
      <AdminPageHeader title={VIEW_META.dashboard.title} />
      ...
      <Card className="content-card dashboard-date-card" size="small">
        ...
        <div className="dashboard-period-summary" aria-live="polite">
          <Text strong>{dashboardPeriodLabel(anchorDate, granularity, customRange)}</Text>
...
function NotificationScreen(props: NotificationScreenProps) {
  ...
  return (
    <>
      <AdminPageHeader
        title={VIEW_META.notifications.title}
...
function StoreScreen({ stores, setStores, brands }: { stores: StoreRecord[]; setStores: React.Dispatch<React.SetStateAction<StoreRecord[]>>; brands: BrandRecord[] }) {
  ...
  return (
    <>
      <AdminPageHeader title={VIEW_META.stores.title} action={closeMode
...
function QrParsingScreen({ onBack }: { onBack: () => void }) {
  ...
  return (
    <>
      <AdminPageHeader title={VIEW_META["qr-parsing"].title} action={<Button icon={<ArrowLeftOutlined />} onClick={onBack}>브랜드 관리로</Button>} />
...
function BrandScreen({ brands, dictionaries, setBrands, onOpenQrParsing }: { brands: BrandRecord[]; dictionaries: DictionaryRecord[]; setBrands: React.Dispatch<React.SetStateAction<BrandRecord[]>>; onOpenQrParsing: () => void }) {
  ...
  return (
    <>
      <AdminPageHeader title={VIEW_META.brands.title} action={<Space className="page-heading-actions" wrap><Button icon={<CodeOutlined />} onClick={onOpenQrParsing}>QR 파싱 로직</Button><Button type="primary" icon={<PlusOutlined />} onClick={openCreate}>브랜드 추가</Button></Space>} />
...
function DictionaryScreen({ dictionaries, setDictionaries }: { dictionaries: DictionaryRecord[]; setDictionaries: React.Dispatch<React.SetStateAction<DictionaryRecord[]>> }) {
  ...
  return (
    <>
      <AdminPageHeader title={VIEW_META.dictionary.title} action={<Button type="primary" icon={<PlusOutlined />} onClick={openCreate}>사전 항목 추가</Button>} />
...
const GROUP_ACCOUNT_DIRECTION_OPTIONS: Array<{ label: string; value: GroupAccountDirection }> = [
  { label: "전체", value: "all" },
  { label: "입금", value: "in" },
  { label: "출금", value: "out" },
];

const groupAccountDirectionLabel = (value: GroupAccountTransaction["direction"]) => value === "in" ? "입금" : "출금";

const groupAccountAmountLabel = (record: GroupAccountTransaction) => {
  const prefix = record.direction === "in" ? "+" : "-";
  return `${prefix}${record.amount.toLocaleString("ko-KR")}원`;
};

const groupAccountDateLabel = (value: string) => {
  const date = new Date(value);
  return Number.isNaN(date.getTime()) ? value : new Intl.DateTimeFormat("ko-KR", { dateStyle: "medium", timeStyle: "short" }).format(date);
};

function GroupAccountScreen() {
  ...
  const loadTransactions = useCallback(async () => {
    if (!status || status.state === "unconfigured") return;
  ...
  // Data loading is an external synchronization; keep the async boundary explicit for the hooks rule.
  useEffect(() => {
  ...
  const columns: TableProps<GroupAccountTransaction>["columns"] = [
    { title: "거래일시", dataIndex: "occurredAt", width: 190, render: (value) => groupAccountDateLabel(value) },
    { title: "내용", dataIndex: "description", render: (value) => <Text strong>{value}</Text> },
    { title: "구분", dataIndex: "direction", width: 90, render: (value) => <Tag color={value === "in" ? "green" : "red"}>{groupAccountDirectionLabel(value)}</Tag> },
    { title: "거래금액", dataIndex: "amount", width: 140, align: "right", render: (_, record) => <Text className={`group-account-amount ${record.direction === "in" ? "is-in" : "is-out"}`}>{groupAccountAmountLabel(record)}</Text> },
    { title: "거래 후 잔액", dataIndex: "balanceAfter", width: 140, align: "right", render: (value) => value == null ? "—" : `${Number(value).toLocaleString("ko-KR")}원` },
  ];

  if (statusLoading) return <LoadingPanel label="계좌 연결 상태를 확인하고 있어요" />;
  if (statusError) {
    return (
      <>
        <AdminPageHeader title={VIEW_META["group-account"].title} action={<Button icon={<ReloadOutlined />} onClick={() => void loadStatus()}>새로고침</Button>} />
    ...
    return (
      <>
        ...
        <Card className="content-card group-account-status-card">
          ...
          <Title level={3}>계좌 연결 정보가 없습니다.</Title>
        </Card>
      </>
    );
  }
  ...
  return (
    <>
      <AdminPageHeader title={VIEW_META["group-account"].title} action={<Button icon={<ReloadOutlined />} loading={transactionsLoading || statusLoading} onClick={() => void loadTransactions()}>새로고침</Button>} />
      <Card className="content-card group-account-toolbar-card">
        <div className="group-account-account-row">
          ...
          <Text type="secondary">최근 조회 {status.lastSyncedAt ? groupAccountDateLabel(status.lastSyncedAt) : "—"}</Text>
        </div>
        <div className="group-account-filter-row">
          ...
          <Segmented<GroupAccountDirection>
            options={GROUP_ACCOUNT_DIRECTION_OPTIONS}
            value={direction}
            disabled={transactionsLoading}
...
function AdminWorkspace() {
  ...
  const analytics = useAnalyticsMetrics(view === "analytics");
