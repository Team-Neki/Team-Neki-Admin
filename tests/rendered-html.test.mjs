...
test("keeps the Neki design foundation and API adapter boundary explicit", async () => {
  ...
  const brandScreen = adminApp.slice(adminApp.indexOf("function BrandScreen"), adminApp.indexOf("type PoseFilter"));
  const poseScreen = adminApp.slice(adminApp.indexOf("function PoseScreen"), adminApp.indexOf("function AdminWorkspace"));
  assert.equal(brandScreen.match(/<Checkbox\.Group\b/g)?.length, 3);
  assert.match(brandScreen, /<Checkbox\.Group<BrandQrFilter>[\s\S]*name="brand-android-qr-filter"[\s\S]*value=\{androidQrFilters\}[\s\S]*aria-label="Android QR 지원 필터"/);
  assert.match(brandScreen, /<Checkbox\.Group<BrandQrFilter>[\s\S]*name="brand-ios-qr-filter"[\s\S]*value=\{iosQrFilters\}[\s\S]*aria-label="iOS QR 지원 필터"/);
  ...
  assert.match(brandScreen, /const matchesAndroidQr = androidQrFilters\.length === 0[\s\S]*androidQrFilters\.includes\("supported"\) && brand\.androidQrSupported[\s\S]*androidQrFilters\.includes\("unsupported"\) && !brand\.androidQrSupported/);
  assert.match(brandScreen, /const matchesIosQr = iosQrFilters\.length === 0[\s\S]*iosQrFilters\.includes\("supported"\) && brand\.iosQrSupported[\s\S]*iosQrFilters\.includes\("unsupported"\) && !brand\.iosQrSupported/);
  ...
  assert.match(brandScreen, /return matchesQuery && matchesAndroidQr && matchesIosQr && matchesMap/);
  assert.match(brandScreen, /const brandFiltersActive = Boolean\(androidQrFilters\.length \|\| iosQrFilters\.length \|\| mapFilters\.length\)/);
  assert.match(brandScreen, /const filtersActive = Boolean\(query \|\| brandFiltersActive\)/);
  assert.match(brandScreen, /const resetBrandFilters = \(\) => \{ setAndroidQrFilters\(\[\]\); setIosQrFilters\(\[\]\); setMapFilters\(\[\]\); setPage\(1\); \}/);
  ...
  assert.match(brandScreen, /pagination=\{\{ current: page, pageSize: 5, responsive: true, showLessItems: true, showSizeChanger: false,[\s\S]*onChange: setPage \}\}/);
